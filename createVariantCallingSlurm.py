...
for line in open(params.fp):
    if re.findall(r'gatk=', line): params.gatk = line.split('=')[-1].rstrip()
    elif re.findall(r'bgzip=', line): params.bgzip = line.split('=')[-1].rstrip()
    elif re.findall(r'email=', line): params.email = line.split('=')[-1].rstrip()
    elif re.findall(r'sleep=', line): params.sleep = line.split('=')[-1].rstrip()
    elif re.findall(r'tabix=', line): params.tabix = line.split('=')[-1].rstrip()
    elif re.findall(r'htslib=', line): params.htslib = line.split('=')[-1].rstrip()
    ...
    elif re.findall(r'cpu_bamvcf=', line): params.cpu_bamvcf = line.split('=')[-1].rstrip()
    elif re.findall(r'tmp_dir=', line): params.tmp_dir = line.split('=')[-1].rstrip()
    ...
    elif re.findall(r'software_dir=', line): params.software_dir = line.split('=')[-1].rstrip()
...
#    if os.path.exists(params.output_dir + '/' + genome + '/' + genome + '.merged.txt'): fp = genome + '.merged.txt'
#    elif os.path.exists(params.output_dir + '/' + genome + '/' + genome + '.realign.txt'): fp = genome + '.realign.txt'
#    else: exit()
	
#    for success in open(params.output_dir + '/' + \
#                        genome + '/' + fp):
#        if re.findall(r'SUCCESS', success):
    ...
    # bamvcf.write("module load python/" + params.python + "\n")
    bamvcf.write("\n")
